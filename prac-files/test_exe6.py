...
def initialise():
    ...
    BrickPi.SensorType[PORT_1] = TYPE_SENSOR_LIGHT_OFF
    BrickPiSetupSensors()

def forward(distance):
    motorRotateDegree([153,150],[27.8*distance,27.8*distance],[LEFT_MOTOR,RIGHT_MOTOR])

def turn(degrees):
    motorRotateDegree([150,150],[-2.45*degrees,2.45*degrees],[LEFT_MOTOR,RIGHT_MOTOR])
...
def characterize_location(ls, learning):
    ...
    if learning:
        ...
        ls.sig = shapeList(ls.sig,360)
        print "rotate back"
        motorRotateDegree([200],[7.15*180],[SONAR_MOTOR],[SONAR_SENSOR],ls,False)
...
def cal_angle(ls_obs, ls_read):
    ...
    for i in range(len(reads)/2):
        distance = 0
        distance = sum(pow(array(reads[i:i+len_obs])-array(obs),2))
...
def navigateToWaypoint(x, y):
    ...
    [degree, distance] = calibration(x, y)
    print "fdsafasf"+str([degree, distance])
    global hit
    while distance > 2:
        ...
        if distance > 30:
            forward(30)
            #particles.draw()
            motion = Coordinate(forwardDis = 30)
        else:
            forward(distance)
            #particles.draw()
...
if __name__ == "__main__":
    ...
    current_theta = angle
    print "at location" + str([current_x,current_y,current_theta])
