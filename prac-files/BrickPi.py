...
def motorRotateDegree(power,deg,port,port_sonar=None,location_signature=None,read=None,sampling_time=.02):
        ...
        if not result :
            if read :
                ...
                if abs(current_value - pre_value)/10>0:
                    ...
                    dis = BrickPi.Sensor[port_sonar[0]]
                    location_signature.sig.extend([float(dis)])
            ...
            for i in range(num_motor):                #Do for each of the motors
                ...
                if(deg[i]>0 and final_val[i]>init_val[i]) or (deg[i]<0 and final_val[i]<init_val[i]) :    #Check if final value reached for each of the motors
                    ...
                    run_stat[i]=1
                    if port[i] == PORT_D:
                        BrickPi.MotorSpeed[port[i]]=int(-power[i]*1.22) if deg[i]>0 else power[i]    
                    else:
